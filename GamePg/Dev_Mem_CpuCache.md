...
CPUはメモリとは別にキャッシュという専用メモリを持っています。今回はその機構のことをCPUキャッシュと呼ぶこととします。
...
初っぱなからまとめです。このまとめの意味が分かる方は以降の説明は読まなくて大丈夫です。
...
func() を呼び出すとdataSの内容がdataDにコピーされます。「memcpy使えばいいのでは？」とか「static変数は元から0だよ？」というツッコミがきそうですが今は気にしないでください。

まず大前提として、メモリにデータを書き込んだりメモリからデータを読み込む処理というのはそのたびに一定のオーバーヘッドがかかります。ですので、メモリへのアクセスする回数が減れば減るほど処理時間が短くなります。

上の例では1000回dataSの内容を読み込み、1000回dataDに書き込む処理を行っています。これを素直に実現するとメモリへのアクセスが読み込みが1000回と書き込みが1000回発生していまい2000回分のオーバーヘッドがかかってしまいます。
...
先ほどの例のコードにもどりますが、CPUキャッシュがない場合とある場合でこのような挙動の差ができます。
...
- CPUキャッシュ有り（動作イメージ）
  ...
  - 最後にCPUキャッシュの内容を dataS のアドレスにコピーする。（メモリ書き込みが1回発生）

CPUキャッシュがある場合、メモリ読み込みと書き込みは1回ずつで済みました。そのためメモリアクセスのオーバーヘッドも減り、処理時間もより短くなるという結果になりました。
...
CPUキャッシュにメモリからデータを読み込んだり、CPUキャッシュのデータをメモリへ書き込んだりするタイミングは不定です。正確には、OS、CPU、コンパイラの組み合わせで変わるため、コードを組む段階では予測不能です。

## メモリの内容と処理結果が不定になるケース

CPUからしかメモリ操作しないようなプログラムであればCPUキャッシュがメモリアクセスするタイミングを意識する必要は全くありません。
...
GPUが描画に使用するテクスチャを、 createWhiteImage() を使って生成したとします。ですが、この処理の結果がCPUキャッシュ上では完成されていたとしても、その結果がメモリに書き込まれているかどうかは不定です。もしメモリに書き込まれていない状態でGPUがこのテクスチャを参照すると、白ではない画像として処理されるかもしれません。

**ケース2. GPU が描画結果をフレームバッファに書き込む**
...
{
    clearFrameBuffer(aImagePtr, aImageSize); // aImagePtr の内容を既定値クリア
    drawScene(aImagePtr, aImageSize); // aImagePtr に対してGPUで描画する関数（という想定）
    saveToBitmap(aImagePtr, aImageSize); // 描画結果を画像ファイルに出力
}
```

GPUの描画で使うフレームバッファイメージを clearFrameBuffer() で初期化してから drawScene() でGPUを使い描画。そして描画結果を saveToBitmap() でビットマップファイルとして保存するという内容です。
...
あるいは、CPUキャッシュ上に aImagePtr の内容が存在している場合、最新のメモリ上のデータをCPUキャッシュにコピーしなくなるため、GPUの描画結果という最新のメモリ上のデータを取得したつもりが古いデータを取得し正しいビットマップ画像を出力できない、といったこともありえます。
...
{
    clearFrameBuffer(aImagePtr, aImageSize); // aImagePtr の内容を既定値クリア
    CacheFlush(aImagePtr, aImageSize); // クリアした結果を確実にメモリに反映
    drawScene(aImagePtr, aImageSize); // aImagePtr に対してGPUで描画する関数（という想定）
    CacheInvalidate(aImagePtr, aImageSize); // GPUの処理結果をメモリから確実に読み込むための準備
    saveToBitmap(aImagePtr, aImageSize); // 描画結果を画像ファイルに出力
}
```

このように適所で関数を呼んでやることで、処理の結果が不定になってしまう事態を防ぐことができます。

## おわり

GPUやDSPの処理結果が何かおかしなことになった場合、CPUキャッシュまわりでミスしている可能性も疑ってみてください。
